...
async def export_to_pdf(
    ...
    if include_summary:
        summary_text = f"""
        <b>Total Molecules:</b> {len(molecules)}<br/>
        <b>Pareto Optimal:</b> {sum(1 for m in molecules if m.is_pareto)}<br/>
        <b>Starred:</b> {sum(1 for m in molecules if m.is_starred)}<br/>
        """
        if molecules:
            avg_eff = sum(m.efficacy_score for m in molecules) / len(molecules)
            avg_saf = sum(m.safety_score for m in molecules) / len(molecules)
            avg_env = sum(m.environmental_score for m in molecules) / len(molecules)
            summary_text += f"""
            <b>Average Efficacy:</b> {avg_eff:.1f}<br/>
            <b>Average Safety:</b> {avg_saf:.1f}<br/>
            <b>Average Environmental:</b> {avg_env:.1f}<br/>
            """
    ...
    if include_scores:
        ...
        data = [headers]
        for mol in molecules[:100]:  # Limit to 100 for PDF
            smiles_short = mol.smiles[:30] + "..." if len(mol.smiles) > 30 else mol.smiles
            data.append([
                str(mol.id),
                smiles_short,
                f"{mol.efficacy_score:.0f}",
                f"{mol.safety_score:.0f}",
                f"{mol.environmental_score:.0f}",
                f"{mol.sa_score:.0f}"
            ])
